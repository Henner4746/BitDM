...
off: **325 bytes in 1.35 seconds.** The radio underneath was measured
separately on the same two phones: **120 ms to find the peer, 700 bytes in
...
| Nearby transport, no network | **proven one direction** | 325 B / 1.35 s between two devices, both offline; independently witnessed by an ESP32 rig acting as a foreign device |
...
| Desktop client | **builds, not linked** | Windows binary runs; it cannot join an existing identity — see [Known gaps](#known-gaps) |
...
| | State | Nearby over BLE | Push wake-up | Lock factors offered |
|---|---|---|---|---|
...
- **Who talks to whom, at a relay you do not control.** A relay cannot read
  ...
  they send. Two answers exist. Self-hosting, which is one command. And the
  **nearby-only** switch, which opens no relay connection at all — no
...
published hash byte for byte — it will be signed with *your* key, not ours.
That is the expected result, not a failure.

The Windows output is a directory, not a single file. `bitdm.exe` is 87 KB and
...
```bash
cd secure-messenger/app && flutter test      # 807 tests: app + crypto + nearby
cd secure-messenger && py server/test_relay.py && py server/test_blob.py   # 35 + 20
```
...
**Two traps when building the Android APK on Windows,** each worth three failed
attempts if you hit it cold:
...
**One thing up front: everyone involved needs the same relay.** There is no
federation between instances. Two people on different relays cannot message
each other.
...
```
secure-messenger/
  app/lib/               Flutter app — 64 files, 21,364 lines Dart
    main.dart              4,217 of them; the platform switches live here
  ...
  app/test/              72 files, 16,481 lines — 807 tests, all green
...
- **The web build is not finished.** It compiles and it lays out; it is not a
  client you should rely on.
...
that sees who is online when. The AGPL closes that gap, which is what makes
"run it yourself" a promise instead of a slogan.

For a piece of software whose entire claim is that you do not have to trust the
operator, being readable is a precondition, not a courtesy.
